...
description: AEM 6.3 - Versionshinweise zur kumulativen Fehlerbehebung
...
# AEM 6.3 Versionshinweise zu den insgesamt Fix Pack-Versionen {#release-notes-aem-cumulative-fix-pack}
...
|---|---|
| **Version** | 6.3 |
| **Version** | Cumulative Fix Pack 6.3.3.8 auf [PackageShare](https://www.adobeaemcloud.com/content/marketplace/marketplaceProxy.html?packagePath=/content/companies/public/adobe/packages/cq630/cumulativefixpack/AEM-CFP-6.3.3.8), [Software Distribution(Beta)](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq630/cumulativefixpack/aem-6.3.3-cfp-8.0.zip) |
| **Voraussetzung** | [AEM 6.3 Service Pack 3 (6.3.3.0)](https://helpx.adobe.com/experience-manager/6-3/release-notes/sp3-release-notes.html) |
| **Allgemeine Verfügbarkeit** | 05. März 2020 |
...
Weitere Informationen zu CFP und anderen Versionstypen finden Sie unter [Maintenance Release Vehicle Definitions.](https://docs.adobe.com/content/docs/en/aem/6-3/deploy/maintenance-release-vehicle-definitions.html)
...
Die wichtigsten Highlights des **AEM kumulative Fix Pack** sind:
...
>[!NOTE]
>
>Für AEM Instanzen mit einer Version unter 6.3.3.0 empfiehlt Adobe die Bereitstellung von SP/CFP über den Installationsordner für Kunden, die eine große Anzahl von Benutzern auf AEM Instanz haben.
...
* Auf der Seite &quot;Hinzufügen zur Sammlung&quot;werden nicht alle verfügbaren Sammlungen in der Ansicht &quot;Karte&quot;angezeigt, während Assets zur Sammlung hinzugefügt werden (NPR-32537).

### Sites {#sites}

* Wenn Sie einen Parameter und eine darin enthaltene Komponente auswählen und den Tastaturbefehl zum Löschen ausgewählter Elemente verwenden, löscht die Aktion sowohl die Komponente als auch ihre übergeordneten Parameter (NPR-32071).
...
### Kampagne-Targeting {#campaign-targeting}

* Der Inhalt einer Komponente, der in der Autoreninstanz geändert und dann aktiviert wurde, ist in der Veröffentlichungsinstanz erst sichtbar, wenn die Komponente neu gestartet wird **com.day.cq.personalization.impl.TargetedContentManagerImpl** (NPR-32489 und NPR-32232).
* Contexthub-Performances stürzen beim Veröffentlichen ab (NPR-31170).

### Brand Portal {#brand-portal}

* Adobe I/O ist nicht in Adobe Experience Manager 6.3 für Markenportal (NPR-32056) integriert.
...
* Designer: Wenn die Tagging-Option aktiviert ist, wird der Rand des Teilformulars in der generierten PDF-Ausgabe (NPR-32324 und NPR-32545) ausgeblendet.
* Designer: Wenn eine Tabelle zusammengeführte Zellen enthält, schlägt der Barrierefreiheitstest für die Ausgabe-PDF-Datei fehl, die mithilfe des Output-Dienstes (NPR-32068) aus einem XDP-Formular konvertiert wurde.
...
* Die Variable `${extension}` wird bei der Workflow-Verarbeitung (NPR-31694) nicht im Autorenmodus gerendert.
* Der für den Modus „Dynamic Media Hybrid“ konfigurierte Wert zum Ablauf (Client-Cache-Zeit bis zum Livestream) wird nicht in die Umgebung für die Bereitstellung dynamischer Medien repliziert (NPR-31114) .
...
* Die Seiteneigenschaften einer Masterseite können nicht geladen werden und es wird eine NullPointerException ausgegeben. Das Problem wird durch Hinzufügen der Eigenschaft cq:blueprint behoben. (NPR-30901)
...
* Wenn ein Benutzer sich auf Felder in der linken Schiene konzentriert und zum Einfügen von Inhalten einen Tastaturbefehl verwendet, wird der Inhalt der Zwischenablage des Seiteneditors anstelle des Inhalts eingefügt, der aus den Feldern der linken Schiene kopiert wurde (NPR-31173).
...
* Max. Wiederverwendungs-Anzahl mit JMX-Konsole wird für den HTML2PDF-Dienst nicht angezeigt. (NPR-30305) 

#### Foundation JEE  {#foundation-jee}

* Max. Wiederverwendungs-Anzahl mit JMX-Konsole wird für den HTML2PDF-Dienst nicht angezeigt. (NPR-30136)
...
* Dienst/Datenquelle für die Datenbankverbindung konnte beim Erstellen des Formulardatenmodells (FDM) nicht angezeigt werden. NPR-30108: Hotfix für CQ-4273359

### Forms JEE-Installationsprogramm  {#forms-jee-installer-1}

#### Forms - Document Services {#forms-document-services}

* Wenn ein Ladetest für den HTML-zu-PDF-Dienst ausgeführt wird, schlägt er mit einem Fehler fehl und die Dateitypeinstellungen werden vom AEM Forms-Server entfernt. NPR-30111, NPR-30086: Hotfix für CQ-4271495
...
* (Erlebnisfragmente) FacebookApplication#setUpApp verwendet veraltete API, die nicht mehr funktioniert. NPR-29213: Hotfix für CQ-4266630
...
### OSGI-Bundles und Inhaltspakete sind in 6.3.3.5 enthalten  {#osgi-bundles-and-content-packages-included-in}
...
* CommitFailException löst das Hochladen großer ZIP-Archivelemente aus. NPR-28528: Hotfix für CQ-4260903
...
* Website-übergreifende Skripterstellung (XSS) in SuggestionHandler für 6.3. NPR-28692: Hotfix für CQ-4253821
...
* (MSM) Implementieren Sie LiveCopyIndex mithilfe von oak-index. NPR-29198: Hotfix für CQ-4222472
* Die Datei „coral.js“ enthält eine anfällige Version der Bibliothek „handlebars.js“. NPR-26973: Hotfix für CQ-4255377
* Wenn Sie eine Textkomponente mit einem verschachtelten Layout-Container und einer Textkomponente verwenden, wird der JavaScript-Fehler &quot;Die Eigenschaft currentPos von null kann nicht gelesen werden&quot;ausgegeben, wenn Sie Text bearbeiten oder auf den Container klicken. NPR-29077: Hotfix für CQ-4246594
* (Touch-Benutzeroberfläche) Die Aktualisierung von Tags auf Seiten, die bereits mit verschiedenen Tags versehen sind, kann nicht als Massen-Aktualisierung durchgeführt werden. NPR-28729: Hotfix für CQ-4262922
* Das Öffnen der Variante in der Kartenansicht führt zu einem 500-Fehler. NPR-28611: Hotfix für CQ-4263571
* Das Rollout einer Struktur, die in einem Master verschoben wurde, führt zu einem falschen cq:moveTarget. NPR-28968: Hotfix für CQ-4265280

### Integrationen  {#integration}

* (Cloud Service Configs) Das Kontrollkästchen &quot;geerbt von&quot;auf der Stammebene sollte entfernt werden. NPR-28771: Hotfix für CQ-4259676
...
* Kompilierungsproblem in OfferproxyStandardProvider.java aufgrund fehlender Importanweisung für die „Resource“-Klasse: Fehlende Importanweisung: import org.apache.sling.api.resource.Resource. NPR-28772
...
* PushOnModify-Roll-out für PageManager-Bewegung verpflichtet sich mehrmals von zwei Sitzungen, wodurch eine Racebedingung entsteht. NPR-28880: Hotfix für CQ-4266191
...
### OSGI-Bundles und Inhaltspakete sind in 6.3.3.4 enthalten  {#osgi-bundles-and-content-packages-included-in-1}
...
* Rep hinzugefügt: in Veröffentlichungsinstanzen ignorierbare Knoten beim AEM Communities User Sync Listener zwischenspeichern.
* Es wurde ein aria-label für die Ansicht von Listen und Karten hinzugefügt.
...
* Die ImageServer-Veröffentlichungseinstellungen replizieren nicht den Hybrid-Versand. NPR-28329: Hotfix für CQ-4253030
...
* Es wurde ein aria-label für die Ansicht von Listen und Karten hinzugefügt. NPR-27736: Hotfix für CQ-4254027
* (Backport) Rep: in Veröffentlichungsinstanzen ignorierbare Knoten beim AEM Communities User Sync Listener zwischenspeichern. NPR-27841: Hotfix für CQ-4247234
* Den Sonderzeichen wird auf UI-Ebene im Suchfeld ein Escape-Zeichen (\) vorangestellt. NPR-27839: Hotfix für CQ-4259757
...
* Die Ausführung von Version Purge in AEM 6.3 führt zu einer ständig wiederholten Warnung in den Protokollen. NPR-27750; Hotfix für CQ-4206870
* Das Stil-Plug-in wird im Vollbildmodus des Rich-Text-Editors nicht unterstützt. NPR-27622: Hotfix für CQ-4258674
* Liste &#39;loaderPromises&#39; wird nicht geleert, nachdem der Inhaltsrahmen in editor.js NPR-27768 geladen wurde: Hotfix für CQ-4205337
* Die Vorlagenrichtlinie kann nicht für verschachtelte Parsen festgelegt werden, ohne dass die Einstellung für die übergeordnete Komponente erfolgt. NPR-27987: Hotfix für CQ-4246095
* Der Komponentenbrowser bereinigt Benutzereingaben nicht und kann daher JavaScript-Fehler auslösen. NPR-27986: Hotfix für CQ-4247590
...
* ResourceResolver löst nicht mehrere Domänen für die Target-Komponente auf. NPR-28265: Hotfix für CQ-107847
...
#### Forms - Dokument Services  {#forms-document-services-2}
...
* Eine Ausnahme wird mit dem Namen java.lang.IllegalArgumentException message:Keine Enum-Konstante com.adobe.internal.pdfm.docbuilder.signature.PathValidationFailureReason.SIGNED_IN_FUTURE ausgelöst, die den Abschluss des Prozesses mit kurzer Lebensdauer verhindert. NPR-28652

#### Forms - Adaptive Formulare  {#forms-adaptive-forms}

* hinzufügen/entfernen Elemente in der Dropdown-Liste wird beim Aktivieren der Kontrollkästchen nicht aktualisiert. NPR-28224: Hotfix für CQ-4252834

### Forms - JEE-Installationsprogramm  {#forms-jee-installer-4}

* Keine neuen AEM Forms-Fehlerbehebungen im Forms JEE-Installationsprogramm.

### OSGI-Bundles und Inhaltspakete sind in 6.3.3.3 enthalten  {#osgi-bundles-and-content-packages-included-in-2}
...
* (Externer Link-Checker) Interne Links werden auf einzelnen Seiten als beschädigt angezeigt, bei externen Links funktioniert dies jedoch nicht. NPR-27481: Hotfix für CQ-4257780
...
* Null Pointer-Ausnahme bei Verwendung eines Kernkomponenten-  zusammen mit einem Foundation-Formular. NPR-27333: Hotfix für CQ-4249176
* Rich-Text-Editor entfernt das leere Alt-Tag. NPR-26938: Hotfix für CQ-4253267
* (Klassische Benutzeroberfläche) Leistungsprobleme mit   selektionchanged   Listener bei mehreren Dropdown-Listen. NPR-27115: Hotfix für CQ-4237215
...
* Ein Bild wird nach Angabe von Haltepunkten in der dynamischen Medienkomponente nicht angezeigt. Hotfix für CQ-4256168
...
* Video wird nicht im Viewer-Vorgabeneditor für gemischte Mediensets wiedergegeben. Hotfix für CQ-4251442
...
* Wenn Sie „Priviligierte Mitglieder zulassen“ aktivieren, sollten die privilegierten Mitglieder nur Inhalte für Benutzer erstellen können, die Mitglieder der Community sind. NPR-27901: Hotfix für CQ-4251235
...
* Translation Vorschau funktioniert nicht mit den für den Handel bestimmten Beispielinhalten. NPR-27170: Hotfix für CQ-4241179

* Proaktive Fehlerbehebungen für platform.login. NPR-26961
* Speichern und Schließen auf den Seiteneigenschaften kehrt nicht zur  richtige Seite in AEM WAR mit Tomcat. NPR-27567: Hotfix für Granite-23671
...
#### Forms - Dokument Services  {#forms-document-services-3}
...
* (Omniture Search) Leistungsproblem bei der Suche innerhalb einer Sammlung. NPR-26714: Hotfix für CQ-4253408
...
* Deaktivieren Sie &quot;Firma-Stammordnerpfad&quot;in der Benutzeroberfläche der Dynamic Media Cloud-Konfiguration. NPR-26361: Hotfix für CQ-4249505
...
* (DM Hybrid) Die Katalogreplikation bricht für AEM 6.3.2 mit Dynamic Media. NPR-26406: Hotfix für CQ-4251306
...
* (Touch-Benutzeroberfläche) Die Symbolleiste kann mit mehreren Rich-Text-Editoren nicht mehr verwendet werden, während vom Vollbildmodus zum schwebenden Dialogfeld gewechselt wird. NPR-25652: Hotfix für CQ-4206008
...
* Vorgänge zum Verschieben von Tags werden nicht von  Tagfelder für strukturierte Inhaltsfragmentmodelle. NPR-26801: Hotfix für CQ-4251805
* (Touch-Benutzeroberfläche) Das Rückgängigmachen der Veröffentlichung der untergeordneten Seite im Seiteneditor funktioniert nicht, nachdem die Seite im Entwurf umbenannt wurde. NPR-26774: Hotfix für CQ-4254175
* Unveröffentlicht  Seite funktioniert nicht mit Verweisen. NPR-26749: Hotfix für CQ-4254372
* Wenn Sie eine Änderung als Live-Kopie erstellen, muss der Benutzer die Seite aktualisieren, damit sie angezeigt wird. NPR-26663: Hotfix für CQ-4254328
* (Klassische Benutzeroberfläche) Wenn Sie zu den Seiteneigenschaften zurückkehren, verwendet das Miniaturbild nicht mehr Vererbung und verschwindet vom Site-Administrator und Sidekick und das Bild wird als leer angezeigt. NPR-26562: Hotfix für CQ-4252346
* Wenn eine Version einer Seite erstellt und ein Vergleich ausgelöst wird, werden die Knoten von /content/ versionshistory sind in der Liste der Live-Kopien für das Blueprint aufgeführt. NPR-26506: Hotfix für CQ-4243957
...
* (Sicherheitslücke) Kontoübernahme aufgrund einer JWT-Fehlkonfiguration. NPR-26440: Hotfix für CQ-4253314
...
* Verhalten bei Klicks zur Abschnittpagnierung in Communities-Kommentaren und beim Laden von Seiten. NPR-27030: Hotfix für CQ-4251228
* (Forumsuche) Die Schaltfläche Zurück überspringt die Seite und wechselt zurück zu forum.html. NPR-26949: Hotfix für CQ-4254804
* Die Ungelesen-Benachrichtigung ermöglicht keine höhere Anzahl als 21. NPR-26947: Hotfix für CQ-4251251
* (SearchScheduledPosts-Auftrag) Hinzufügen den Schalter in ConfigMgr aktivieren/deaktivieren. NPR-26924: Hotfix für CQ-4250463
...
* Suchproblem in der Forumkomponente. NPR-26577: Hotfix für CQ-4251303
...
* (Forumssuche) - Paginierte Links zu Threads mit vielen Beiträgen funktionieren nicht bei Benachrichtigungen. Hotfix für CQ-4254202
...
* Null Zeigerausnahme beim Hinzufügen von Gruppen und Mitgliedern zu Rolleneinstellungen und beim Zulassen des privilegierten Mitglieds in der Blog-Funktion. NPR-21732, NPR-27176: Hotfix für CQ-4255909
...
* Fehlerbehebungen zur proaktiven Plattformanmeldung. NPR-26961
* (Multifield) Lassen Sie Composite-Elemente benutzerdefinierte Namen zu. NPR-26493: Hotfix für Granite-20568
...
* (IE11) &quot;NaN&quot;wird im Zahlenfeld angezeigt, wenn ein negativer Wert eingegeben wird. NPR-26701: Hotfix für CQ-100826
* (Koral. Multifield) Verschachtelte Mehrfachfelder verwenden die falsche Vorlage, um die Elemente zu erstellen. NPR-25649: Hotfix für CUI-6743
* Aktualisieren Sie die clientlibs &quot;granite coralui2&quot;und &quot;coralui3&quot;, um die Handlebars aus dem Build zu entfernen. NPR-25606: Hotfix für Granite-22116

### Formulare {#forms-7}

### Forms-Add-on-Paket {#forms-add-on-package-7}

#### Forms - Dokument Security {#forms-document-security-2}
...
#### Forms - Dokument Services  {#forms-document-services-4}
...
* Eine Null Pointer-Ausnahme wird ausgelöst, wenn ein nicht mit einem vorausgefüllten adaptiven Formular/Formularsatz verknüpfter Startpunkt geöffnet wird. Hotfix für CQ-4256124
...
* Wenn Sie die Suchvorlage über die Admin-Benutzeroberfläche festlegen und dann dieselbe für die Suche nach der Aufgabe in AEM Forms Workspace verwenden, wird eine Ausnahme ausgelöst. Hotfix für CQ-4255649
* Die Verfolgungsdetails der Prozesse unter den Anwendungen mit Klammern-Symbol im Namen werden im HTML-Arbeitsbereich nicht angezeigt. Hotfix für CQ-4242101
...
* Ungültige Fehlermeldungen in der Anwendungsverwaltung. NPR-23289: Hotfix für CQ-4233163, CQ-4255636
* Fehler beim Klicken auf „Authentifizierung hinzufügen“ während der Konfiguration der Domain. Hotfix für CQ-4255634
...
#### OSGI-Bundles und Inhaltspakete sind in 6.3.3.1 enthalten  {#osgi-bundles-and-content-packages-included-in-3}
...
* Die Render-Bedingung gibt ein gesundes Ergebnis zurück, wenn expressionResolver null ist.
...
* (Metadata-Profil) Die zum Zeitpunkt der Erstellung festgelegten Standard-Tag-Werte sind nach dem Speichern nicht mehr verfügbar. NPR-23370: Hotfix für CQ-4235458
* (Touch-Benutzeroberfläche) Aufgrund des JS-Fehlers können nicht mehrere Assets verschoben werden. NPR-23395: Hotfix für CQ-4241279
...
* (Firefox/Chrome) Assets können nicht auf der Seite &quot;Asset-Freigabe&quot;heruntergeladen werden. NPR-23963: Hotfix für CQ-4244391
...
* (Brand Portal) Die Suche in der Ordnervorhersage sollte bei der Replikation herausgefiltert werden. NPR-23292: Hotfix für CQ-4241385
...
* (Markenportal) Aktivieren Sie die Erstellung von Teilassets, wenn ein Asset veröffentlicht wird. NPR-23331: Hotfix für CQ-4242018
* Anfragen für dynamische Medien verwenden keinen allgemeinen Proxy/HTTP-Client; NPR-10727: Hotfix für CQ-45695, CQ-88800
...
* (Seiten-Editor) Bild in einer Bildkomponente wird beim erneuten Öffnen des Dialogfelds ausgeblendet. NPR-23156: Hotfix für CQ-4239978
...
* (Rich Text Editor) - Der Vorgang &quot;Ersetzen&quot;ändert das Format eines Wortes. NPR-23271: Hotfix für CQ-4239677
...
* Fehler &quot;Ungültiger Rekursionsauswahlwert&quot;bei der Migration einer Komponente von AEM 6.0 auf AEM 6.2. NPR-23532: Hotfix für CQ-4241258
* (Rich Text Editor) QuickInfos zeigen den Variablennamen des Plugins anstelle des lesbaren Plugin-Namens an. NPR-23550: Hotfix für CQ-4243269
...
* (Zielgruppe) Kampagnen werden nicht aktiviert, wenn der API-Typ als XML verwendet wird. NPR-23199: Hotfix für CQ-4240936****
...
* (OMEGA) Bericht &quot;Funktion&quot;nur auf Englisch. NPR-23990: Hotfix für Granite-21231
...
* (ContextPath) Die Breaking-Benachrichtigungsfunktionalität schlägt bei Jetty-Konfigurationen und bei der Suche nach Gruppenmitgliedern in der Liste der Community-Gruppenmitglieder fehl. NPR-23336: Hotfix für CQ-4241519, CQ-4242080
...
* Probleme beim Ausführen von  Suchen Sie nach der Forumkomponente. NPR-23838: Hotfix für CQ-4243770
* (Community Moderation Gekennzeichnet) Die Massengenehmigung von gekennzeichneten Nachrichten funktioniert nicht. NPR-23845: Hotfix für CQ-4243962
* Sortieren-Schaltflächentext zeigt nicht den standardmäßig ausgewählten  value inspite   der Auswahl der Standardsortierreihenfolge. NPR-23881: Hotfix für CQ-4243375
...
* Fehlerbehebungen bei mehreren serverseitigen AEM Communities-Problemen. NPR-23971: Hotfix für CQ-4243144, CQ-4242145, CQ-4243365, CQ-4244098
...
* Beim Ausrollen der Seite zeigt die Livecopy nicht die vom Übergeordnete geerbten An-/Ausschaltzeiteigenschaften an. NPR-23873: Hotfix für CQ-4243431
...
* Der Paper Capture-Dienst stürzt bei gleichzeitiger Verarbeitung von TIFF-Dateien ab (Größe ca. 50 KB). NPR-23556
...
### OSGI-Bundles und Inhaltspakete sind in 6.3.2.2 enthalten  {#osgi-bundles-and-content-packages-included-in-4}
...
* (Touch-Benutzeroberfläche) Standardmäßiger Upload-Speicherort für Assets in der Ansicht der Spalten. NPR-22465: Hotfix für CQ-4237057
...
* (DAM) Mehrere Site-übergreifende Skripterstellungs-(XSS-)Schwachstellen beim Hinzufügen zum Sammlungsassistenten. NPR-22432: Hotfix für CQ-4327086
...
* Das neue Element im Workflow-Posteingang zeigt den Seitenpfad anstelle des Titels der Seite an. NPR-21634: Hotfix für CQ-4230672
...
* (TouchUI) Mehrere Site-übergreifende Skripting-(XSS-)Schwachstellen für HTML-Komponenten. NPR-21899: Hotfix für CQ-4232511
* Der Ressourcentyp für Inhaltsfragmente mit gemischten Medien kann nicht geändert werden. NPR-21907: Hotfix für CQ-4233401
* Das RET-Vollbild verdeckt bei der Touch-optimierten Benutzeroberfläche die RTE-Plug-ins. NPR-22034: Hotfix für CQ-4235457
* (Touch-Benutzeroberfläche) Der Rich-Text-Editor entfernt alle Attribute außer ID aus dem &lt;a>-Tag. NPR-22044: Hotfix für CQ-4234133
...
* (Klassische Benutzeroberfläche) Die Ausgabe des Rich-Text-Editors html fügt &#39;Liste-Position-Stil: inside;&#39; als Inline-Stil für das &lt;ul>-Tag. NPR-22145: Hotfix für CRTE-114
...
* (Klassische Benutzeroberfläche) Das Einfügen von Text-Shortcut (Strg+V) funktioniert nicht für OOTB Text (Rich Text)-Komponenten. NPR-22224: Hotfix für CQ-4236224
* Das Filtern von Tagfield  funktioniert bei der Eingabe von Text nicht wie erwartet. NPR-22236: Hotfix für CQ-4236655
* (Seiten-Editor) Beim Einfügen von Textdaten in die Imagemap-Komponente wird die Textkomponente auch beim Einfügen von Textdaten in die Imagemap-Komponente eingefügt. NPR-22264: Hotfix für CQ-4236230
...
* (Touch-optimierte Benutzeroberläche) Das Löschen von Launches über die Konsole der klassischen Benutzeroberfläche macht alle Seiten unbearbeitbar. NPR-22491: Hotfix für CQ-4225074
...
* Beim Öffnen der Komponente im  im Inlide-Modus sind zuvor geladene Plugins beim zweiten Mal nicht sichtbar. NPR-22591: Hotfix für CQ-4236850
...
* (Classic UI Sidekick) Die Registerkarte &quot;Workflow&quot;ist deaktiviert, wenn sich die Seite im Workflow-Sperrstadium befindet. NPR-22722: Hotfix für CQ-4237557
* Nach dem Spiegeln eines Bildes, das der Bildkomponente auf einer Seite hinzugefügt wurde, werden die Änderungen nicht gespeichert und das  Originalbild wird auf der Seite angezeigt. Rendering-Unterstützung wurde der Image-Core-Komponente über [https://github.com/Adobe-Marketing-Cloud/aem-core-wcm-components/pull/141](https://github.com/Adobe-Marketing-Cloud/aem-core-wcm-components/pull/141) hinzugefügt. NPR-22801: Hotfix für CQ-4221539
...
* Aktivieren einer Seite mit  Instanzen im Veröffentlichungsmodus werden in den grünen Status umgewandelt, anstatt in Bernstein umzuschalten. NPR-22927: Hotfix für CQ-4236310
* (StyleSystem) Die Position des Bildschirms springt, während der Stil aus dem Popup ausgewählt wird. NPR-23183: Hotfix für CQ-4238867
...
* PersonalizationDataSource löst eine Null Pointer-Ausnahme aufgrund eines fehlenden  jcr : content-Knoten. NPR-22850: Hotfix für CQ-4222122
* AEM Targeting schlägt bei der Verwendung von  nicht englische Sprache. NPR-22917: Hotfix für CQ-4218213
...
* Benutzer können keine statischen Param-Werte im  mbox-Aufruf, der beim Testen mit AT.js als Client-Bibliothek in der Cloud-Konfiguration angezeigt wird. NPR-21930: Hotfix für CQ-4234520
...
* Stabilitätsprobleme mit Asset Share AEM Herausgeberinstanz aufgrund von Speicherlecks. NPR-22205, NPR-23178: Hotfix für Sling-5668, Sling-7292 und Sling-7470.
* Die instabile Service-ID sollte nicht für Sitzungs-Attributnamen verwendet werden. NPR-22821: Hotfix für Granite-21059
* Wenn  ein http   wenn die von Whiteboard verwaltete Sitzung ungültig ist, wird die Sitzungssitzung auch ungültig, wenn die Sitzung keine anderen Sitzungsattribute hat. NPR-23059: Hotfix für FELIX-5819
...
* Wenn Sie eine Seite aktivieren, die ein gelöschtes Experience Fragment enthält, wird eine Null Pointer-Ausnahme ausgelöst. NPR-23179: Hotfix für CQ-4239939

### Projekte {#projects}

* (Mehrsprachiges Projekt) Das Projekt Liste keine Übersetzungsaufträge für mehr als 19 Sprachen. NPR-22498: Hotfix für CQ-4229978
...
* Benutzerdefinierte Funktionen für den Regeleditor fügen zusätzliche Funktionen hinzu. nach Funktionsaufruf, sodass die Überprüfung fehlschlägt, obwohl die benutzerdefinierte Funktion true zurückgibt. NPR-22481: Hotfix für CQ-4235499
...
* (Microsoft Edge) (IE11) Bei Textfeldkomponenten für adaptive Formulare mit mehrzeiligen Feldern wird als Standardwert &quot;Null&quot;anstelle von &quot;leer&quot;angezeigt. NPR-22284: Hotfix für CQ-69107
...
* (Firefox) Die XML-Schema-Eigenschaften des adaptiven Forms können nicht aktualisiert werden, da die Optionen im Dokument des Datensatzes (DOR) auf der Eigenschaftenseite nicht vorausgewählt sind. NPR-22298: Hotfix für CQ-4237402
...
* OOTB-Basisauthentifizierung für SOAP-Dienste funktioniert nicht für einfache Authentifizierung in der FDM-Integration. NPR-23238: Hotfix für CQ-4241308
...
* Diskrepanz zwischen Acrobat DC- und AEM Berichten über den Fehler &quot;PDF/A-1b-Konformitätsprüfung&quot;. NPR-22051, NPR-22050: Hotfix für CQ-4226128, CQ-4227671
...
* (HTML Workspace) Probleme mit der Bildschirmgröße bei Bildlaufleisten. NPR-23288
* (HTML Workspace) Prozessstartpunkte werden nicht in alphanumerischer Reihenfolge sortiert. NPR-22841 Hotfix für CQ-4238944
* (HTML Workspace) Vorbereitende Daten werden beim Schließen des Formulars in Workspace ausgelöst. NPR-21127: Hotfix für CQ-4224574
* (HTML Workspace) Wenn Sie einen Prozess aufrufen, für den Notizen mit langen Beschreibungen erforderlich sind, funktioniert die Schaltfläche zum Erweitern der Notizen nicht. Hotfix für CQ-4241488
...
#### Dienst für Unterschriften  {#signatures-service}
...
### OSGI-Bundles und Inhaltspakete sind in 6.3.2.1 enthalten  {#osgi-bundles-and-content-packages-included-in-5}
...
* Ersetzen Sie Versionsvergleichscode durch Adobe-proprietären Code aufgrund von Lizenzproblemen. NPR-20272: Hotfix für CQ-4223758
...
* Der Metadaten-Fehler der DAM-Asset-API bricht die xmp -write-back-APIs. NPR-20607: Hotfix für CQ-4220455
* Leistungsprobleme beim Hinzufügen von Benutzern zu Sammlungen. NPR-20699: Hotfix für CQ-4225733
* Die Veröffentlichung über AEM in Brand Portal ist für dynamische Mediensets nicht zulässig. NPR-20320: Hotfix für CQ-4221147
...
* Das E-Mail-Bundle von Apache-Kommas `{org.apache.commons/commons-email/1.5}` wurde hinzugefügt und ersetzt `{com.day.commons.osgi.wrapper/com.day.commons.osgi.wrapper.commons-email/1.2.0-0002}`.
...
* Die OOTB-Granitbenutzer-Auswahl lädt keine Benutzer, wenn im Schritt Dialogteilnehmer des Workflow-Modells auf Dropdown-Liste geklickt wird. NPR-20236
...
* Die Unterstützung für Aggregations-Ausdruck für wiederholte Bereiche fehlt. NPR-20861
* Dropdown zeigt den zuletzt gespeicherten Wert an, auch wenn der zugehörige Formular-Datenmodelldienst keinen Wert ausgibt. NPR-20710
* Die vorhandenen Regeln können im Regeleditor nicht mit booleschen Einschränkungen bearbeitet werden. NPR-21128
...
* Verwalten Sie die Listen zum Senden von Workflows nur in zehn Anwendungen. CQ-4230193

#### HTML5-Formulare {#html-forms-3}

* Der Name eines XDP-Formularobjekts wie eines Teilformulars wird als QuickInfo angezeigt, wenn es nicht in den Barrierefreiheitskonfigurationen definiert ist. NPR-20523
...
* Installieren Sie die neueste CTJPEG2K-Bibliothek für eine kritische Sicherheitslücke. Dies wirkt sich auf die Module XMLFM (AEM und IfBA), RM und PDFG aus. NPR-20625: NPR-21337.
...
### OSGI-Bundles und Inhaltspakete sind in 6.3.1.2 enthalten  {#osgi-bundles-and-content-packages-included-in-6}
...
FileAEM Cumulative Fix Pack 6.3.1.1 ist ein wichtiges Update, das seit der allgemeinen Verfügbarkeit von AEM 6.3 Service Pack 1 (6.3.1.0) im Oktober 2017 einige interne und kundenspezifische Fehlerbehebungen enthält.
...
* dc: title und dc: Die Beschreibung ändert sich nicht in einen Wert für mehrere Felder in  crx /de. NPR-19570: HF für CQ-4209086
...
* Der Adobe-E-Mail-Vorlagendienst fügt benutzerdefinierten Benutzervorlagen Tags hinzu. NPR-19190
...
* UTF-8-Zeichen als Ausgabe des SOAP-Dienstes werden für nicht englischsprachige Gebietsschemas nicht korrekt in adaptive Formularfelder kopiert. NPR-20238: NPR-20103
...
#### Assembler-Dienste  {#assembler-services}
...
* Anlagen, Notizen und Details des Arbeitsablaufs funktionieren nicht im Arbeitsbereich, wenn das Formular maximiert/minimiert und als Entwurf gespeichert oder weitergeleitet wird. NPR-20243
...
#### Standarddienste {#standard-services}

* docConvertor unterstützt keine Reduzierungstransparenzen in PDF und erzeugt keine PDF/A-Datei. NPR-16228: Hotfix für CQ-4214488

#### Core  {#core-2}

* Wenn der AEM Forms-Server, der in einer Cluster-Konfiguration auf der JBoss-Anwendung ausgeführt wird, beendet wird, wird der Anwendungsserver von der Datenbank getrennt. Dies kann zu Datenbeschädigung führen. NPR-19724
...
### OSGI-Bundles und Inhaltspakete sind in 6.3.1.1 enthalten  {#osgi-bundles-and-content-packages-included-in-7}
...
* Vereinfachte Konfiguration für intelligente Inhaltsdienste
...
* Probleme mit der Benutzerfreundlichkeit bei der Konfiguration intelligenter Inhaltsdienste mit verschiedenen Optionen. NPR-18200: Hotfix für CQ-4201557
...
* Eine Null Pointer-Ausnahme tritt auf, wenn ein Asset von seiner Eigenschaftsseite verschoben wird. CQ-4204251
...
* Geänderte lokalisierte Komponenten, die nicht in ihrem ursprünglichen Formular wiederhergestellt werden, wenn die Vererbung in einer LiveCopy erneut angewendet wird. NPR-18172: Hotfix für CQ-4211379
...
* Nicht geschlossener Ressourcenauflöser in den folgenden Objekten:
   ...
   * `com.day.crx.delite.impl.AuthHttpContext` NPR-17493: GFP für GRANITE-17404
...
AEM Forms-Fehlerbehebungen werden über Forms-Add-on-Pakete und andere mit der Version gelieferte Patch-Installationsprogramme bereitgestellt. Weitere Informationen finden Sie unter [AEM Forms Releases](aem-forms-releases.md).
...
* Der PDF Generator-Dienst kann Bilder, die größer als 100 MB sind, nicht in PDF-Dokumente konvertieren. Referenznummer CQ-4208628
* Bei Verwendung des PDF Generator-Dienstes mit Japanisch-OCR (Zeichenerkennung) wird eine invertierte PDF generiert. NPR-17602
...
AEM Cumulative Fix Pack 6.3.0.1 ist ein wichtiges Update, das seit der allgemeinen Verfügbarkeit von AEM 6.3 im April 2017 mehrere interne und kundenspezifische Fehlerbehebungen enthält. Die wichtigsten Highlights des AEM Cumulative Fix Pack sind:

* Verbesserungen in folgenden Bereichen:

   * Inhaltsfragmente, Sites und Rich-Text-Editor-, Regel-Editor- und Vorlageneditor-Komponenten
...
* Bietet eine Überprüfungsoption im Package Manager zum Erkennen von Konflikten zwischen Überlagerung und CFP

## Download-Anweisungen für CFP über Softwareverteilung {#download-instructions-for-cfp-via-package-share}

Sie können das CFP-Paket direkt von [Software-Distribution](https://www.adobeaemcloud.com/content/marketplace/marketplaceProxy.html?packagePath=/content/companies/public/adobe/packages/cq630/cumulativefixpack/AEM-CFP-6.3.3.8) herunterladen oder die folgenden Schritte ausführen:
...
>
>Sie können unter [https://docs.adobe.com/content/docs/en/aem/6-3/administer/content/package-manager.html#Package%20Validator](https://docs.adobe.com/content/docs/en/aem/6-3/administer/content/package-manager.html#Package%20Validator) auf die Dokumentation für die Option &quot;Validate&quot;zugreifen.
...
* Sie können eine neue Protokollfunktion am Standardspeicherort [http://localhost:4502/system/console/slinglog](http://localhost:4502/system/console/slinglog) mit den folgenden Eigenschaften hinzufügen:

   * Protokollebene: Debug
   * Additive: false
   * Log File: logs/activity.log
   * Protokollfunktion: org.apache.jackrabbit.vault.processing.impl.ActivityLog

Aktivität.log wird erstellt in   Ordner crx -quickstart/logs.

### Installieren Sie das kumulative Fix Pack über die Softwareverteilung {#install-the-cumulative-fix-pack-via-package-share}
...
* Verwenden Sie die [HTTP-API von Package Manager](https://helpx.adobe.com/de/experience-manager/6-3/sites/administering/using/package-manager.html) - stellen Sie sicher, dass Sie `cmd=install&recursive=true` verwenden - damit das verschachtelte Paket installiert ist.

### Bestätigen der Installation {#validate-installation}

1. Auf der Seite &quot;Produktinformationen&quot;( `/system/console/  productinfo`) sollte nun die aktualisierte Versionszeichenfolge &quot;Adobe Experience Manager, Version 6.3.3.8&quot;unter &quot;Installierte Produkte&quot;angezeigt werden.
1. Alle OSGI-Bundles sind entweder ACTIVE oder FRAGMENT in der OSGI-Konsole (Web-Konsole verwenden: `/system/console/bundles`).

>[!NOTE]
>
>Bei erfolgreicher Installation des Pakets wird in den Fehlerprotokollen eine Informationsmeldung angezeigt, die darauf hinweist, dass das Inhaltspaket erfolgreich installiert wurde, z. B. &quot;Inhaltspaket **AEM-6.3.3-Cumulative-Fix-Pack-7** erfolgreich installiert.&quot;

>[!NOTE]
>
>Seit AEM 6.3.3.1 hat sich die Protokollierungsstufe in Jackrabbit FileVault für die meisten Meldungen von INFO in DEBUG geändert. Um Installationsprotokolle für ein auf AEM 6.3.3.1 oder höher installiertes Inhaltspaket abzurufen, aktivieren Sie die DEBUG-Protokollierungsstufe für org.apache.jackrabbit.vault.packages.impl.
...
1. Installieren Sie das Forms Add-On-Paket, wie unter [Installieren AEM Forms Add-On-Pakete](https://helpx.adobe.com/de/experience-manager/6-3/forms/using/installing-configuring-aem-forms-osgi.html) beschrieben.
...
1. Um das Update zu installieren, führen Sie die Datei Designer 6.2.0__Cumulative_QF.msp aus.
...
* Die folgenden Fehler und Warnungen können während der Installation von AEM CFP 6.3.3.x auftreten und können unbedenklich ignoriert werden:

   * *WARN* [OsgiInstallerImpl] org.apache.jackrabbit.vault.processing.impl.InstallHookProcessorImpl Hook /META-INF/vault/hooks/cloudservices-wfchangeinstallaken-0.0.2-jar-with-dependencies.jar gibt eine Laufzeitausnahme aus.
   * *ERROR* [OsgiInstallerImpl] com.adobe.cq.social.cq-social-jcr-provider [com.adobe.cq.social.provider.jcr.impl.SpiSocialJcrResourceProviderImpl(2174)] Timeout wartet darauf, dass der reg-Wechsel nicht registriert wird. CQ-4209974.
   * org.apache.sling.engine.impl.SlingRequestProcessorImpl ServletResolver-Dienst fehlt, kann keine Service-Anforderungen senden, Sendestatus 503
   ...
   * *ERROR* [FelixDispatchQueue] com.day.cq.dam.cq-dam-core FrameworkEvent ERROR (org.osgi.framework.ServiceException: Service Factory gab null zurück. (Component: com.day.cq.dam.handler.standard.ps.PostScriptHandler))
...
>[!NOTE]
>
>Falls Sie von einem dieser beiden Probleme betroffen sind, wenden Sie sich an den [AEM Kundendienst](https://helpx.adobe.com/marketing-cloud/contact-support.html).
...
>[!NOTE]
>
>Diese Konfigurationseinstellung gilt für CFP 6.3.3.2 und höher. Es weist Sie an, die Eigenschaften der Bootdelegation in der Datei `sling` .properties zu aktualisieren.
...
* Öffnen Sie adobe-livecycle-cq-author.ear/cq.war/WEB-INF/web.xml zur Bearbeitung:
   ...
   * aktualisieren Sie den Wert für den Parameter **sling.bootdelegation.ibm** mit:
...
* Deinstallieren Sie die vorherige EAR-Datei (Enterprise Archiv) vom Websphere-Anwendungsserver und installieren Sie die aktualisierte EAR-Datei, wie in Abschnitt 10.2 von [https://helpx.adobe.com/pdf/aem-forms/6-3/install-single-server-websphere.pdf](https://helpx.adobe.com/pdf/aem-forms/6-3/install-single-server-websphere.pdf) beschrieben.
* Speichern Sie die Datei und starten Sie den Server neu. [https://helpx.adobe.com/pdf/aem-forms/6-3/install-single-server-websphere.pdf](https://helpx.adobe.com/pdf/aem-forms/6-3/install-single-server-websphere.pdf)

## Für NPR-23208 erforderliche Konfigurationseinstellungen {#configuration-settings-required-for-npr-1}

>[!NOTE]
>
>Diese Konfigurationseinstellung gilt für 6.3.2.2 und höher. Es leitet Sie an, die ACL-Richtlinien (Zugriffskontrolle Listen) manuell über CRX-DE zu aktualisieren, da ACLs aufgrund von &quot;merge_preserve&quot; acHandling nicht durch die CFP-Installation aktualisiert werden.
...
`1)` Pfad &quot;/content&quot;\
`a)` Grundsatz: reference-Adjustment-Service\
Typ: Zulassen\
Berechtigungen: jcr:read , jcr:modifyProperties\
Einschränkungen: rep:glob=&quot;/*/jcr:content&quot;\
`b)` Grundsatz: reference-Adjustment-Service\
Typ: Zulassen\
Berechtigungen: jcr:read , jcr:modifyProperties\
Einschränkungen: rep:glob=&quot;/*/jcr:content/*&quot;
...
Die Eigenschaft steuert die maximale Tiefe des Knotenunterbaums unter dem Knoten ` /  jcr   :content`, bis zu dem die im Repository vorhandenen Knoten zum Abrufen der Seiteneigenschaften verwendet werden. Jeder Knoten, der unter der angegebenen Tiefe in dieser Eigenschaft vorhanden ist, wird ignoriert.

Der Standardwert ist 1. Der Wert kann  überschreibt, indem die Datei &quot;constants.js&quot;(`/libs/cq/ui/widgets/source/constants.js`) die Kommentierung der Eigenschaft &quot;CQ.PAGE_PROPERTIES_MAX_RECURSION_LEVEL&quot;aufgehoben und ihr der erforderliche Wert zugewiesen wird ( die maximale Tiefe unter der Eigenschaft &quot;/jcr:content&quot;der Seite, bis zu der die Daten der Seiteneigenschaften gespeichert werden).

**Wenn der Benutzer mehrere Seitenvarianten erstellen muss, sodass die Anzahl der Knoten unter dem Knoten / jcr :content größer als 1000 ist, führen Sie die folgenden Schritte aus, um Konfigurationsänderungen vorzunehmen:**

* Konfigurieren Sie die Eigenschaft „JSON Max results“ von Apache Sling
* Servlet abrufen mit `/system/console/  configMgr`
* Setzen Sie den Wert auf eine Zahl >1000 (der aktuelle Standard), sodass diese Zahl größer ist als die Gesamtanzahl der Knoten in der Unterstruktur / jcr :content, bis die konfigurierte Tiefe oben erreicht ist.

Dadurch kann das sling GET-Servlet alle erforderlichen Knoten zurückgeben.

## Uber Jar {#uber-jar}

Die Uber-Jar für 6.3.3.8 ist unter [Adobe Public Maven Repository](https://repo.adobe.com/nexus/content/groups/public/com/adobe/aem/uber-jar/6.3.3.8/) verfügbar.

Um Uber Jar in einem Maven-Projekt zu verwenden, lesen Sie den Artikel [Wie Sie Uber jar](https://helpx.adobe.com/de/experience-manager/6-3/sites/developing/using/ht-projects-maven.html) verwenden und fügen Sie die folgende Abhängigkeit in Ihr Projekt-POM ein:
...
>[!MORELIKETHIS]
>
>* [Versionen und Aktualisierungen von AEM](https://helpx.adobe.com/experience-manager/aem-releases-updates.html)
>* [AEM 6.3 Hotfixes-Seite](https://helpx.adobe.com/de/experience-manager/kb/aem63-available-hotfixes.html)
